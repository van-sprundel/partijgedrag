...
export function AnalysisPage() {
	...
	return (
		<div className="container mx-auto p-4">
			...
			{activeTab === "coalition" && (
				<CoalitionTab period={period} parties={parties || []} />
			)}
			{activeTab === "deviations" && <DeviationsTab period={period} />}
...
function CoalitionTab({
	period,
	parties,
}: { period: string; parties: Party[] }) {
	const { data: alignments, isLoading } = useCoalitionAlignment(period);
...
function DeviationsTab({ period }: { period: string }) {
	const { data: deviations, isLoading } = useMPDeviations(period, 25);
...
function TopicsTab({ period }: { period: string }) {
	const { data: trends, isLoading } = useTopicTrends(period);
