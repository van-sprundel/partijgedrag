...
export const statisticsRouter = {
	getPartyLikenessMatrix: os.statistics.getPartyLikenessMatrix.handler(
		async ({ input }) => {
			...
			const reverseResults = rawResults.map((r) => ({
				party1Id: r.party2Id,
			...
			const allResults = [...rawResults, ...reverseResults];

			return allResults.map((r) => ({
				party1Id: r.party1Id ?? "",
				party1Name: r.party1Name ?? "",
				party2Id: r.party2Id ?? "",
				party2Name: r.party2Name ?? "",
				commonMotions: Number(r.commonMotions ?? 0),
				sameVotes: Number(r.sameVotes ?? 0),
				likenessPercentage: Number(r.likenessPercentage ?? 0),
			}));
		},
	),
